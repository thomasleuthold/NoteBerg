...
- **Bind failure is surfaced, not swallowed.** The port is fixed (8765), so if another process already holds it the listener never binds and the server simply isn't there. `enabled` can't detect this — it's the user's intent, pushed in from JS and stored whether or not a listener exists — so `McpState` carries a separate `listening` flag, set only after a successful bind and reported by `mcp_get_status`. Both UI surfaces gate on `enabled && listening`: the footer badge stays hidden, and Settings shows a distinct "port already in use, close it and restart" warning. That warning is deliberately *not* the existing `mcpStatusMismatch` one, which offers a Retry button — retrying a config push cannot free a taken port, so reusing it would present a button that can't work.
- **An XSS in the webview is a full MCP compromise.** `mcp_set_config` is a normal Tauri command with no caller restriction, so any JS running in the webview can call it and enable the server with an attacker-chosen token — bypassing Settings, the keyring, and the persisted `mcp_enabled` flag, none of which Rust ever cross-checks. This is inherent to the "JS is the source of truth" invariant (ADR-002): closing it properly means moving persistence into Rust, which is a larger change than it buys today given the trusted-local-session model. It is recorded here so the escalation path is explicit rather than implied. The CSP is the mitigating control — hence `connect-src 'self' ipc: http://ipc.localhost`, which closes off webview-native exfiltration of note content to an arbitrary host. This costs nothing functionally: in the Tauri build every remote request already goes through `@tauri-apps/plugin-http` (Rust-side, CSP-exempt), and the native `fetch` calls in `storage.webdav.js` belong to the Nextcloud-app build, which this CSP never governs. The **Rust** HTTP allowlist (`capabilities/*.json`) deliberately stays broad (`http://**`, `https://**`, plus the `*:*` port forms — see below): the Nextcloud server is user-configured and self-hosted instances on plain HTTP are supported, so narrowing it would break real setups rather than buy security. Narrowing to private IP ranges (`10.x`, `192.168.x`, `172.16-31.x`, `*.local`) was built and verified working, then **rejected**: the scope test runs on the URL string before DNS resolution, so a bare LAN hostname (`http://nextcloud/`, `http://mynas/nextcloud`, `http://nextcloud.fritz.box/` — the latter a default router domain) is indistinguishable from a public host by pattern and would silently lose sync. Injected JS could still reach the network via the plugin, so `connect-src` is a partial mitigation, not a seal. Note also that `note.content` reaches MCP via `text_html` as raw HTML by design, so it is not a sanitization boundary. `'unsafe-eval'` remains required by the `perspective-transform` shim (`src/shims/perspective-transform.js`), which evaluates the upstream UMD source via `new Function` at runtime; removing it means giving the native build the same build-time evaluation the NC build already gets (`vite.config.js`). pdf.js is **not** the blocker — the `jbig2_nowasm_fallback` inlining in `vite.config.js` exists precisely so pdf.js does *not* need `wasm-unsafe-eval`.

  **`**` does not cover the port — both forms are required.** In `tauri-plugin-http`'s URLPattern matcher, `**` wildcards the *hostname* only; the port component defaults to the scheme's default (80/443), so `http://**` matches `http://host/x` but **not** `http://host:5089/x`. Collapsing the allowlist to just `http://**`/`https://**` therefore broke every explicit-port caller — the recognition sidecar on `localhost:5089` (observed live: `url not allowed on the configured scope`) and, silently, any LAN Nextcloud on a non-default port. The `*:*` entries restore explicit-port matching. Verified against the plugin's real `scope.rs` matcher across both port forms for localhost, LAN IPs, `.local`, and public https.
...
- **`Origin` header validation** (spec-required, DNS-rebinding protection): any request carrying an `Origin` header is rejected (403). Real MCP client libraries are not browser pages and don't send `Origin` on plain HTTP requests — only a browser-originated fetch (the rebinding attack) would carry one. Verified live: a curl request with `Origin: https://evil.example` gets 403; the same request without it succeeds. This check runs **first, ahead of the enabled/token checks**: a rebinding page is by definition tokenless, so behind the token check it would be rejected as a 401 before ever reaching the 403 — making it dead code in the exact scenario it exists for. Ordering it first is what makes it genuine defense in depth rather than a layer that only appears active.
- The bearer-token auth from Phase 2 sits **underneath** the JSON-RPC layer, checked before any request body is read or dispatched — matches the spec's own local-server guidance (`security_best_practices` §"Local MCP Server Compromise": HTTP transport SHOULD require an auth token). It remains the real security boundary; the `Origin` check preceding it is an independent layer, not a replacement.
- **Request bodies are capped at 1 MiB** (`MAX_BODY_BYTES`), returning **413** on overflow. Inbound bodies are small JSON-RPC envelopes, so the cap is pure headroom; the reason it matters is that the MCP server shares a process with the app itself, so an unbounded `read_to_string` would be an app-wide OOM — losing the user's editing session, not merely the server. The read takes one byte past the cap so an oversized body is reported honestly as 413 instead of being silently truncated into a confusing parse error.
